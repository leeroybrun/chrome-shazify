angular.module('myApp', ['ngRoute'])
	...
	.factory('StorageHelper', function() {
		...
		Storage.prototype.get = function(names, callback) {
			...
			chrome.storage.sync.get(toGetFromStorage, function(items) {
				for(var key in items) {
					var name = key.replace(storage.prefix+'_', ''); // Retrive original name

					data[name] = items[key];
					storage.cache[name] = items[key];
				}
		...
		Storage.prototype.set = function(objects, callback) {
			...
			for(var key in objects) {
				...
				data[this.prefix+'_'+key] = objects[key];
			}
			...
			chrome.storage.sync.set(data, function() {
				...
				if(chrome.runtime.lastError) {
					...
					console.log('An error occured during storage set: '+ error);
				}
	...
	.factory('TagsService', function(StorageHelper) {
		var Tags = {
			list: [],
	...
	.factory('ShazamService', function(ChromeHelper, StorageHelper, TagsService, $timeout, $http) {
		var Shazam = {
			newTags: [],
	...
	.factory('SpotifyService', function(ChromeHelper, StorageHelper, Helper, TagsService, $timeout, $http) {
		var Spotify = {
			api: {
	...
	.directive('chromeTranslate', function() {
	    return {
	        restrict: 'A',
	...
	.controller('TagsCtrl', function($scope, $location, $timeout, ShazamService, SpotifyService, TagsService) {
	...
	.controller('SettingsCtrl', function($scope, $location, ShazamService, SpotifyService) {
		$scope.shazam = {
			loginStatus: false,
